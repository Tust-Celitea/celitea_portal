...
from .. import db
from ..models import Permission, Role, User, Post, Comment,Group,Registration
...
def index():
    return render_template('index.html', groups=Group.query.order_by(Group.name).all())

@main.route('/showcase', methods=['GET', 'POST'])
def showcase():
    return render_template('showcase.html', groups=groups)

@main.route('/helloclass', methods=['GET', 'POST'])
def helloclass():
    return render_template('helloclass.html', groups=groups)
...
def portal():
    ...
    return render_template('portal.html', form=form, posts=posts,
...
def follow(username):
    ...
    if current_user.is_following(user):
        flash('汝已经和他/她/它在一个小组啦~')
        return redirect(url_for('.user', username=username))
    current_user.follow(user)
    flash('汝现在和 %s 在一个小组啦!' % username)
    return redirect(url_for('.user', username=username))
...
def unfollow(username):
    ...
    if not current_user.is_following(user):
        flash('汝没和他/她/它在一个小组啦~')
        return redirect(url_for('.user', username=username))
    current_user.unfollow(user)
    flash('汝现在不和 %s 在一个小组啦!' % username)
    return redirect(url_for('.user', username=username))
